...
class Board:
    def __init__(self):
        ...
        self.tokens = {
            Player.red.value: [],   # -1 means token is in yard
    ...
    def move_token(self, player: str, token_index: int, steps: int) -> bool:
        ...
        # Check if token should enter home run
        if (player == Player.blue.value and current_pos <= 50 and current_pos + steps > 50) or \
            (player == Player.green.value and current_pos <= 24 and current_pos + steps > 24):
